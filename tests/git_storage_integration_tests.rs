...
fn test_command_store_serialization() {
    ...
    store.workflows.insert("test-workflow".to_string(), workflow);
...
fn test_git_manager_config_persistence() {
    ...
    let loaded_configs: Vec<RepoConfig> = serde_json::from_str(&content).expect("Should parse config");
...
fn test_command_merge_behavior() {
    ...
    local_commands.insert("cmd1".to_string(), Command::new(
        "cmd1".to_string(),
    ...
    repo_commands.insert("cmd1".to_string(), Command::new(
        "cmd1".to_string(),
        "Repo version".to_string(),
        "echo repo".to_string(),
        vec![],
    ));
    repo_commands.insert("cmd2".to_string(), Command::new(
        "cmd2".to_string(),
